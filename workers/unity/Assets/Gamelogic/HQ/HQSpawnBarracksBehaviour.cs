...
namespace Assets.Gamelogic.HQ
{
    public class HQSpawnBarracksBehaviour : MonoBehaviour
    {
        ...
        private readonly IDictionary<GameObject, HealthBehaviour> barracksDictionary = new Dictionary<GameObject, HealthBehaviour>();
        private float barracksSpawnRadius;

        private void OnEnable()
        {
            barracksSpawnRadius = SimulationSettings.DefaultHQBarracksSpawnRadius;
            spawnBarracksPeriodicallyCoroutine = StartCoroutine(SpawnBarracksPeriodically(SimulationSettings.SimulationTickInterval * 5f));
        ...
        {
            for (int i = 0; i < hqInfo.Data.barracks.Count; i++)
            {
                ...
                if (barracksEntityObject != null)
                {
                    ...
                    if (!barracksDictionary.ContainsKey(barracksGameObject))
                    {
                        barracksDictionary.Add(barracksGameObject, barracksGameObject.GetComponent<HealthBehaviour>());
                    }
                }
            }
        }
        ...
        private IEnumerator SpawnBarracksPeriodically(float interval)
        {
            while (true)
            {
        ...
        private bool AllBarracksAtFullHealth()
        {
            ...
            bool allBarracksFullHealth = true;
        ...
        private Vector3 FindSpawnLocation()
        {
            while (true)
            {
                for (int attemptNum = 0; attemptNum < SimulationSettings.HQBarracksSpawnPositionPickingRetries; attemptNum++)
                {
        ...
        {
            foreach (GameObject barracks in barracksDictionary.Keys)
            {
