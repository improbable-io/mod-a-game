...
namespace Assets.Gamelogic.EntityTemplate
{
    ...
    {
        public static Entity CreatePlayerEntityTemplate(string clientWorkerId, Coordinates initialPosition, uint teamId)
        {
            ...
            template.Add(new PlayerControls.Data(Coordinates.ZERO));
            ...
            template.Add(new Improbable.Chat.Chat.Data());
            ...
            var permissions = Acl.Build()
                ...
                .SetWriteAccess<Improbable.Chat.Chat>(CommonPredicates.PhysicsOnly)
        ...
        public static SnapshotEntity CreateBarracksTemplate(Coordinates initialPosition, BarracksState barracksState, uint teamId)
        {
            ...
            template.Add(new FSimAuthorityCheck.Data());
            template.Add(new TransformComponent.Data(new TransformComponentData(initialPosition, (uint)(UnityEngine.Random.value * 360))));
            template.Add(new BarracksInfo.Data(new BarracksInfoData(barracksState)));
            template.Add(new Health.Data(new HealthData(barracksState == BarracksState.CONSTRUCTION_FINISHED ? SimulationSettings.BarracksMaxHealth : 0, SimulationSettings.BarracksMaxHealth, true)));
            template.Add(new Flammable.Data(new FlammableData(false, false, FireEffectType.BIG)));
            template.Add(new StockpileDepository.Data(new StockpileDepositoryData(barracksState == BarracksState.UNDER_CONSTRUCTION)));
            template.Add(new NPCSpawner.Data(new NPCSpawnerData(barracksState == BarracksState.CONSTRUCTION_FINISHED, new Map<NPCRoleEnum, float> { { NPCRoleEnum.LUMBERJACK, 0f }, { NPCRoleEnum.WIZARD, 0f } })));
        ...
        public static SnapshotEntity CreateNPCWizardSnapshotTemplate(Coordinates initialPosition, uint teamId)
        {
            ...
            template.Add(new TargetNavigation.Data(false, new Option<Coordinates>(), new Option<EntityId>()));
            template.Add(new Spells.Data(new Map<SpellType, float> { { SpellType.LIGHTNING, 0f }, { SpellType.RAIN, 0f } }, true));
            template.Add(new NPCWizard.Data(WizardFSMState.StateEnum.IDLE, EntityId.InvalidEntityId, new NPCRole(NPCRoleEnum.WIZARD)));
            template.Add(new TeamAssignment.Data(new TeamAssignmentData(teamId)));
            template.Add(new TeamKnowledge.Data(new Improbable.Collections.List<Coordinates>(SimulationSettings.TeamHQLocations)));
        ...
        public static SnapshotEntity CreateHQEntitySnapShotTemplate(Coordinates initialPosition, uint initialRotation, uint teamId)
        {
            var template = new SnapshotEntity { Prefab = SimulationSettings.HQPrefabName };
            template.Add(new FSimAuthorityCheck.Data());
            template.Add(new HQInfo.Data(new Improbable.Collections.List<EntityId>()));
            template.Add(new TransformComponent.Data(new TransformComponentData(initialPosition, initialRotation)));
            template.Add(new Health.Data(new HealthData(SimulationSettings.HQMaxHealth, SimulationSettings.HQMaxHealth, true)));
            template.Add(new TeamAssignment.Data(new TeamAssignmentData(teamId)));
