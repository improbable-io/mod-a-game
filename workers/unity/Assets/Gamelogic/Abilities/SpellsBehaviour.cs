...
namespace Assets.Gamelogic.Abilities
{
    public class SpellsBehaviour : MonoBehaviour
    {
        ...
        private void OnEnable()
        {
            ...
            reduceCooldownsCoroutine = StartCoroutine(ReduceCooldowns(1f));
        }
        ...
        private void ApplySpellEffectOnTargets(SpellType spellType, int targetCount)
        {
            ...
            {
                var targetEntityId = spellTargets[i].gameObject.EntityId();
        ...
        private IEnumerator ReduceCooldowns(float interval)
        {
            while (true)
            {
